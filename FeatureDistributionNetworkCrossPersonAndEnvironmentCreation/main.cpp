...
int main(int argc, char** argv) {
	...
	ofstream fSif("CommonFeatureCrossPersonAndEnvironment.sif");
	...
	for (int i = 0; i < samples.size(); i++){
		for (int j = i + 1; j < samples.size(); j++){
			...
			if (s1->sampleName.find("Environment") != string::npos && s2->sampleName.find("Environment") != string::npos) continue;
			if (matrix[i][j] > 0.18){
				int num;
				if ((s1->sampleName.find("Person") == string::npos && s1->sampleName.find("Environment") == string::npos)
					|| (s2->sampleName.find("Person") == string::npos && s2->sampleName.find("Environment") == string::npos)) num = 150;
				else num = 30;
				...
				for (auto &e: res){
					...
					if (s1->sampleName.find("Person") == string::npos && s1->sampleName.find("Environment") == string::npos) Union(feature1, "Control");
					if (s2->sampleName.find("Person") == string::npos && s2->sampleName.find("Environment") == string::npos) Union(feature2, "Control");
				}
			}
		}
	}
	...
	for (auto &from: has){
		for (auto &to: from.second){
			if (Find(to) != "Control" && counter[Find(to)] > 3 && from.first.find("Person") != string::npos) output.insert(from.first + " pf " + Find(to));
			if (Find(to) != "Control" && counter[Find(to)] > 3 && from.first.find("Environment") != string::npos) output.insert(from.first + " ef " + Find(to));
		}
	}
	for (auto &s: output)
		fSif << s << endl;
