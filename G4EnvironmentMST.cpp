...
int main(int argc, char** argv) {
	...
	ofstream fout("G4EnvironmentMinimumSpanningTree.sif");
	...
	for (int i = 0; i < path.size(); i++){
		if (name[i].find("Person") != string::npos && name[i].find("group4") != string::npos && (name[mostSimilarID[i]].find("Person") != string::npos
			|| name[mostSimilarID[i]].find("Environment") != string::npos))
			validPerson.push_back(true);
		else validPerson.push_back(false);
		if (name[i].find("Environment") != string::npos && name[i].find("group4") != string::npos && (name[mostSimilarID[i]].find("Person") != string::npos
			|| name[mostSimilarID[i]].find("Environment") != string::npos))
