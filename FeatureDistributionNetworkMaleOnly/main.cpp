...
int main(int argc, char** argv) {
	...
	ofstream fSif("CommonFeatureForMaleOnly.sif");
	...
	for (int i = 0; i < samples.size(); i++){
		for (int j = i + samples.size() / 3; j < samples.size(); j++){
			...
			if (matrix[i][j] > 0.15){
				int num;
				if ((s1->sampleName.find("_F_") != string::npos || s1->sampleName.find("Female") != string::npos)
					|| (s2->sampleName.find("_F_") != string::npos || s2->sampleName.find("Female") != string::npos)) num = 500;
				else num = 100;
				...
				for (auto &e: res){
					...
					if (s1->sampleName.find("_F_") != string::npos || s1->sampleName.find("Female") != string::npos) Union(feature1, "Control");
					if (s2->sampleName.find("_F_") != string::npos || s2->sampleName.find("Female") != string::npos) Union(feature2, "Control");
				}
			}
		}
	}
	...
	for (auto &from: has){
		for (auto &to: from.second){
			if (Find(to) != "Control" && counter[Find(to)] > 3 && from.first.find("_F_") == string::npos
				&& from.first.find("Female") == string::npos) output.insert(from.first + " mf " + Find(to));
		}
	}
	for (auto &s: output)
		fSif << s << endl;
